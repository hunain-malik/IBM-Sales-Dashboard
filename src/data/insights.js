...
export function buildInsights(deals, enablements) {
  ...
  const caveat = smallSample && known.length > 0
    ? `Small sample (${a.n} influenced vs ${b.n} not) — read as an early signal, not a proven effect.`
    : null
  ...
  if (summary.totalDeals === 0 && summary.sessionCount === 0) {
    ...
    headline = summary.sessionCount === 0
      ...
      : 'No tracked deals have followed our enablement yet — no influence to report.'
  } else if (known.length === 0) {
    tone = 'early'
    headline = `Enablement preceded ${summary.influencedCount} of ${summary.totalDeals} deals (${fmtPct(summary.influencedCount / summary.totalDeals)}), but there aren't enough closed deals to compare performance yet.`
  } else if (losses.length === 0 && wins.length > 0) {
    tone = 'strong'
    headline = `Influenced deals outperform: ${joinList(wins.map((m) => m.win))}.`
  } else if (wins.length > 0 && losses.length > 0) {
    tone = 'mixed'
    headline = `Mixed results: influenced deals show ${joinList(wins.map((m) => m.win))}, but ${joinList(losses.map((m) => m.loss))}.`
  } else if (losses.length > 0) {
    tone = 'weak'
    headline = `Influenced deals aren't outperforming yet: ${joinList(losses.map((m) => m.loss))}.`
  } else {
    tone = 'even'
    // don't overstate: if win rate / cycle are unknowable, say so
    headline = known.length < metrics.length
      ? 'On what can be measured so far, influenced and non-influenced deals look similar — close more deals to compare win rate and cycle length.'
      : 'Influenced and non-influenced deals are performing about the same so far.'
  }
  ...
  if (tone === 'weak' || tone === 'mixed') {
    asks.push('Review the influenced deals that stalled or lost — check whether session timing, content, or audience needs adjusting before scaling up.')
  }
...
export const toneHeading = {
  strong: 'Influenced deals perform better',
  mixed: 'Influenced deals: mixed results',
  weak: 'Influenced deals: no edge yet',
  even: 'Influenced deals: on par so far',
